...
class ScriptEditingSystem:
    def __init__(self, api_key: str = None):
        ...
        # 系统提示词
        self.system_prompt = """你现在是一个剧本编辑系统。用户输入了场景及简单的限制之后，你可以根据输入创建一个简单的剧本，包括角色、剧情等。你担任调度员的角色，调度之后角色的反应。
    ...
    def run(self):
        ...
        if not self.client:
            ...
            print("请设置环境变量 DEEPSEEK_API_KEY 或在初始化时提供API密钥")
            api_key = input("请输入您的DeepSeek API密钥（或按Enter退出）: ").strip()
            if not api_key:
                return
        ...
        print("💡 使用说明：")
        print("- 请输入场景描述和创作限制")
        print("- 例如：'在一个下雨的夜晚，咖啡店里，两个陌生人因为一本书相遇'")
